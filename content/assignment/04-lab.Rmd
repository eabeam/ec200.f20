...
What does this do? It creates a loop for which local variable ``i'` is first 0, then 1, then 2. Within the loop, it generates `labfor0`, which is equal to 1 if `labforce` equals 0 (not in unierse), it generates `labfor1`, which is equal to 1 if `labforce` equals 1 (not in labor force), and it generates `labfor2`, which is equal to 1 if `labforce` equals 2 (in labor force).
...
<img src="../regression-label.png" alt="labelled Stata output">
...
10. Estimate your model again (from part 8), but now exclude any variables
    that are not statistically significant. How does this affect the
    standard error of $\widehat{\beta_X}$? Your $R^2$? Your adjusted $R^2$? *Note that the purpose of this exercise is to see how standard errors and measures of model fit are affected by the inclusion of non-significant controls. It is **not** to suggest you shoudl only include significant independent variables in regressions.*
...
12. Estimate your multiple linear regression model from
    ...
    in the previous part. (If you already controlled for race/ethnicity
    in your first model, use these categorical variables instead.). How
    do the inclusion of these factors affect your estimates of the
    relationship between $Y$ and $X$?
    
13. Now, add "state fixed effects" to your regression. This is a set of binary 0/1 variables, one for each state, which essentially pull out any mean differences in your dependent variable at the state level - so if your outcome variable is different for people in Oregon vs. New York on average, these variables will take care of it. How
    do the inclusion of these factors affect your estimates of the
    relationship between $Y$ and $X$?
